...
public class FrontServlet extends HttpServlet {
    ...
    public void init() throws ServletException {
        ...
        sourceConfiguration.setLocale(new Locale("en"));
        targetConfiguration.setLocale(new Locale("en"));
    }
    ...
    private void process(Route route, HttpServletRequest request, HttpServletResponse response)
            ...
            for (Method classMethod : clazz.getDeclaredMethods()) {
                if (classMethod.getName().equals(route.getAction())) {
                    if (method == null) {
                        ...
                        throw new ServletException("Multiply methods with one name");
                    }
                }
            }
        }
        ...
        if (param != null && Pattern.matches("[a-z]{2}", param)) {
            session.setAttribute("lang", param);
            sourceConfiguration.setLocale(new Locale((String)session.getAttribute("lang")));
            targetConfiguration.setLocale(new Locale((String)session.getAttribute("lang")));
        }
