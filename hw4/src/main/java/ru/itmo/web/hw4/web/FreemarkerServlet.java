        ...
        if (request.getRequestURI().equals("/")) {
            ...
            try {
                template = freemarkerConfiguration
                        .getTemplate(URLDecoder.decode(request.getRequestURI(), UTF_8) + ".ftlh");
            } catch (TemplateNotFoundException ignored) {
                String path = request.getContextPath() + "404";
                response.sendRedirect(path);
    ...
    private Map<String, Object> getData(HttpServletRequest request) {
        ...
        for (Map.Entry<String, String[]> e : request.getParameterMap().entrySet()) {
            if (e.getValue() != null && e.getValue().length == 1) {
                ...
                if (e.getKey().substring(e.getKey().length() - 3).equals("_id")) {
                    try {
