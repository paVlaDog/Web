...
public class FrontServlet extends HttpServlet {
    ...
    private void process(Route route,
        ...
        Method method = null, methodBefore = null, methodAfter = null;
        for (Class<?> clazz = pageClass; methodBefore == null && clazz != null; clazz = clazz.getSuperclass()) {
            try {
                methodBefore = clazz.getDeclaredMethod(route.getBefore(), HttpServletRequest.class, Map.class);
        ...
        for (Class<?> clazz = pageClass; methodAfter == null && clazz != null; clazz = clazz.getSuperclass()) {
            try {
                methodAfter = clazz.getDeclaredMethod(route.getAfter(), HttpServletRequest.class, Map.class);
            } catch (NoSuchMethodException ignored) {
                // No operations.
            }
        }
        ...
        try {
            doMethod(request, pageClass, methodBefore, page, view);
            doMethod(request, pageClass, method, page, view);
            doMethod(request, pageClass, methodAfter, page, view);
    ...
    private void doMethod(HttpServletRequest request, Class<?> pageClass, Method method, Object page, Map<String, Object> view) throws InvocationTargetException, ServletException {
        try {
            if (method == null) {
                throw new ServletException("Page don't has method \"before\" or \"after\"");
            }
    ...
    private static final class Route {
        ...
        private static final String DEFAULT_BEFORE = "before";
        private static final String DEFAULT_AFTER = "after";
