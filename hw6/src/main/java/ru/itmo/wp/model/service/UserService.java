...
public class UserService {
    ...
    public void validateRegistration(User user, String password, String passwordConfirmation) throws ValidationException {
        ...
        if (Strings.isNullOrEmpty(passwordConfirmation)) {
            throw new ValidationException("passwordConfirmation is required");
        }
        if (!password.equals(passwordConfirmation)) {
            throw new ValidationException("Password confirmation don't equals password");
        }
    }
    ...
    public void validateEnter(String loginOrEmail, String password) throws ValidationException {
        User user = (User)userRepository.findByLoginOrEmailAndPasswordSha(loginOrEmail, getPasswordSha(password));
        if (user == null) {
    ...
    public User findByLoginAndPassword(String login, String password) {
        return (User)userRepository.findByLoginAndPasswordSha(login, getPasswordSha(password));
    }

    public User findByLoginOrEmailAndPassword(String loginOrEmail, String password) {
        return (User)userRepository.findByLoginOrEmailAndPasswordSha(loginOrEmail, getPasswordSha(password));
    }
